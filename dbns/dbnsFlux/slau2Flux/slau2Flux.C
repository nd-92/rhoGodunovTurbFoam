...
void Foam::slau2Flux::evaluateFlux(
    scalar &rhoFlux,
    vector &rhoUFlux,
    scalar &rhoEFlux,
    const scalar &pLeft,
    ...
    const scalar &buiEps) const
{
    ...
    const scalar pTilde = 0.5 * (pLeft + pRight) + 0.5 * (betaLeft - betaRight) * (pLeft - pRight) + sqrt(max(0.5 * (magSqr(ULeft) + magSqr(URight)), SMALL)) * (betaLeft + betaRight - 1.0) * rhoBar * cBar;
...
void Foam::slau2Flux::evaluateFreestreamFlux(
    scalar &rhoFlux,
    vector &rhoUFlux,
    scalar &rhoEFlux,
    const scalar &pLeft,
